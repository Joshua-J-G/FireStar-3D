...
RenderSystem::RenderSystem()
{
	...
	for (UINT driver_type_index = 0; driver_type_index < num_driver_types;)
	{
		res = D3D11CreateDevice(NULL, driver_types[driver_type_index], NULL, NULL, feature_levels,
			num_feature_levels, D3D11_SDK_VERSION, &m_d3d_device, &m_feacher_level, &m_imm_context);
		if (SUCCEEDED(res))
			break;
		++driver_type_index;
	}
	if (FAILED(res))
	{
		throw std::exception("FAILED: RenderSystem");
	}
...
SwapCahinPtr RenderSystem::createSwapChain(HWND hwnd, UINT width, UINT height)
{
	SwapCahinPtr sc = nullptr;
...
bool RenderSystem::compileVertexShader(const wchar_t* file_name, const char* entry_point_name, void** shader_byte_code, size_t* byte_code_size)
{
	...
	{
		cout << "FAILED: a Compiling error has occured err03" << endl;
...
bool RenderSystem::compilePixelShader(const wchar_t* file_name, const char* entry_point_name, void** shader_byte_code, size_t* byte_code_size)
{
	...
	{
		cout << "FAILED: a Compiling pixel error has occured err04" << endl;
		if (error_blob) error_blob->Release();
		return false;
	}
	*shader_byte_code = m_blob->GetBufferPointer();
