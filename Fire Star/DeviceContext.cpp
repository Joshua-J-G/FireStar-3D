...
void DeviceContext::clearRenderTargetColour(const SwapCahinPtr& swap_chain, float red, float green, float blue, float alpha)
{
	FLOAT clear_color[] = { red,green,blue,alpha };
	m_device_context->ClearRenderTargetView(swap_chain->m_rendtargview, clear_color);
	m_device_context->OMSetRenderTargets(1, &swap_chain->m_rendtargview, NULL);
}
...
void DeviceContext::setContantBuffer(const VertexShaderPtr& vertex_shader, const ConstantBufferPtr& buffer)
{
	m_device_context->VSSetConstantBuffers(0, 1, &buffer->m_buffer);
}

void DeviceContext::setContantBuffer(const PixelShaderPtr& pixel_shader, const ConstantBufferPtr& buffer)
{
